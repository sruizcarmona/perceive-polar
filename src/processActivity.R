...
process_polarfile <- function(zipfile, filename, id, maxhr) {
  ...
  if (class(act) == "try-error"){
    act.err <- onerow.df(c(id, rep('activity error (unknown)', length(act.err.names) - 2),file), act.err.names) %>% mutate(date = NA_Date_)
    return(act.err)
  }
  # act hr higher than maxhr
  # if(!is.na(act$maxhr_activity) & act$maxhr_activity > act$maxhr_participant) {
  #   act.err <- onerow.df(c(id,rep('activity error (maxHR too high)',length(act.err.names)-2),file),act.err.names)
  #   return(act.err)
  # }
  # timestamp missing
  if(is.na(act$date)) {
  ...
  act.info <- cbind(id, act, file=file)
  return(act.info)
}
